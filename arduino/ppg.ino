...
void loop() {
  if (Firebase.ready()) {
    Firebase.RTDB.getBool(&fbdo, "recording");
    ...
    if (recording) {
      ...
      Firebase.RTDB.setBool(&fbdo, "recording", false);
      Firebase.RTDB.set(&fbdo, "ppg-database/" + timeStamp, &ppg);
...
String getCurrentDate() {
  ...
  return String(day) + "-" + String(month) + "-" + String(year) + " " + String(hour) + ":" + String(min) + ":" + String(sec);
}